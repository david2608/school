...
export function StudentProvider({children}){
  ...
  const refresh=useCallback(async()=>{setLoading(true);setError('');try{setData(await getStudentExperience(user?.id,activeCourseId))}catch(error){setError(error.message||'Could not load your course.')}finally{setLoading(false)}},[user?.id,isDemo,activeCourseId])
  ...
  const selectCourse=courseId=>setActiveCourseId(courseId)
  return <StudentContext.Provider value={{data,loading,error,refresh,completeLesson,submitAssignment,activeCourseId,selectCourse}}>{children}</StudentContext.Provider>
}
